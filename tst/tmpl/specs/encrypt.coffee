{encryptMaps} = require("../../../tmpl/fn/encrypt")
alias = null
psudo = null


module.exports = ->
# DESCRIBE "tmpl.encrypt", ->
  ...
  DESCRIBE "MAPS", ->
    ...
    IT "Spelling", ->
      expect(encryptMaps("aspesdos", alias)).to.equal("Asbestos")
      DONE()


    IT "Spelling case insensitive", ->
      expect(encryptMaps("Aspesdos", alias, psudo)).to.equal("Asbestos")
      DONE()
    ...
    IT "Pseudonyms", ->
      enc = (input) => encryptMaps(input, psudo)
